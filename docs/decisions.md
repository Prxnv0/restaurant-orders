...
## Decision 30 — Deploy files (Render blueprint + Vercel config) instead of manual console setup (M11)

- **Chose:** Provide `render.yaml` (Render Blueprint) and `vercel.json` (Vercel project config) in the repo, plus a `DEPLOY.md` runbook, instead of relying on the user to click through the platform consoles without documentation. All secrets (`DATABASE_URL`, `DIRECT_URL`, `JWT_SECRET`) are marked `sync: false` in `render.yaml` so they must be set in the Render dashboard, never committed to the repo.
- **Rejected:** Manual-only deployment (clicks in the Render + Vercel consoles with no in-repo config), or auto-deploying directly to a real cloud account from the CLI (would require the user's interactive login and credentials that cannot be provided in this session).
- **Why:** In-repo config files let the user re-deploy the project from scratch by importing the same repo on a different account, or by a teammate, with a single click. The `sync: false` flag on every secret is the explicit contract: secrets belong to the dashboard, the repo carries only the deployment shape. `DEPLOY.md` is the user-facing runbook that turns "create an account, click New, type a build command, type env vars, deploy" into a six-step ordered list with troubleshooting. The `healthCheckPath: /api/health` declaration tells Render to use the existing health endpoint (added in M1) — no new code required.
- **What users see:** `DEPLOY.md` step 2 has a copy-paste-ready list of every env var Render needs and where to find the values. The `vercel.json` `rewrites` block ensures the Vite SPA refresh works on Vercel (otherwise `/orders/123` would 404 on a hard refresh). The `frontend/.env.example` documents the one env var the frontend needs.
- **What this cost:** None — the YAML/JSON are declarative and validated at deploy time. The runbook is documentation, not a runtime dependency.

## Decision 31 — `SameSite=None; Secure` cookies for cross-origin auth (M11) — REVERSED DURING DEPLOY

- **Chose (initial M2):** `SameSite=Lax` cookies. Decision 2 in this file noted "SameSite=Strict prevents most CSRF attacks without requiring a CSRF library" as a property of the auth strategy, but the actual implementation used `Lax` to survive top-level navigations.
- **Reversed (M11):** `SameSite=None; Secure` in production, `SameSite=Lax` in dev. The `Lax` value works for same-origin flows (frontend and backend on the same domain) but **silently drops the cookie on cross-origin `fetch()` calls with `credentials:'include'`** when the frontend (Vercel) and backend (Render) are on different domains. The browser doesn't send an error — it just doesn't include the cookie, so the next request to `/api/dashboard` returns 401 "Authentication required", even though the login itself returned 200.
